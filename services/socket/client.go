...
func bbb() {
	...
	data := []byte("hello")
	pbdata, err := ClientEncode(DefaultProtocolVersion, ActionData, data)
	if err != nil {
		panic(err)
	}
	conn.Write(pbdata)
	...
	for i := 0; i < 3000; i++ {
		data = append(data, []byte(string(i)+" ")...)
	}
...
// ClientDecode :
func ClientDecode(rawConn net.Conn) (*CustomLengthFieldProtocol, error) {

	newPackage := CustomLengthFieldProtocol{}
