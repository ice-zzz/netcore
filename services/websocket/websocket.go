...
func NewEchoServer(addr string) *WebSocket {
...
func NewServer(addr string) *WebSocket {
...
func (server *WebSocket) OnInitComplete(srv gnet.Server) (action gnet.Action) {
...
func (server *WebSocket) OnOpened(c gnet.Conn) (out []byte, action gnet.Action) {
	atomic.AddInt64(&server.ConnNum, 1)
	return
}

func (server *WebSocket) OnClosed(c gnet.Conn, err error) (action gnet.Action) {
	atomic.AddInt64(&server.ConnNum, -1)
	return
}

func (server *WebSocket) React(frame []byte, c gnet.Conn) (out []byte, action gnet.Action) {
...
// 发送下行消息
func (server *WebSocket) SendDownStreamMsg(wsConn *GnetUpgraderConn, opcode ws.OpCode, msg []byte) error {
	if wsConn == nil {
...
// 更新连接活跃时间
func (server *WebSocket) updateConnActiveTs(wsConn *GnetUpgraderConn) {
...
// 关闭连接
func (server *WebSocket) closeConn(wsConn *GnetUpgraderConn) {
	if wsConn == nil {
		return
	}
